...
(function () {
  ...
  function showPostForm(opts) {
    ...
    form.innerHTML =
      ...
      '<div style="display:flex;gap:12px;margin-bottom:14px">' +
        ...
        '</div>' +
        '<div style="flex:1">' +
          '<label style="' + LABEL_CSS + '">\u8F6C\u53D1\u6570</label>' +
          '<input id="kol-pf-shares" type="text" placeholder="0" style="' + INPUT_CSS + '">' +
        '</div>' +
      '</div>' +
      '<div style="margin-bottom:18px">' +
    ...
    document.getElementById('kol-pf-save').addEventListener('click', function () {
      ...
      var shares = parseInt(document.getElementById('kol-pf-shares').value.trim() || '0', 10);
  ...
  function showPostSelector(opts) {
    ...
    title.textContent = opts.title || '\u9009\u62E9\u5E16\u5B50';
    Object.assign(title.style, { fontSize: '15px', fontWeight: '600', color: '#1D1D1F', letterSpacing: '-0.2px' });
    ...
    selectAllBtn.addEventListener('click', function () {
      listEl.querySelectorAll('.kol-ps-cb').forEach(function (c) { c.checked = true; });
    });
    ...
    deselectBtn.addEventListener('click', function () {
      listEl.querySelectorAll('.kol-ps-cb').forEach(function (c) { c.checked = false; });
    });
