...
(function () {
  ...
  function showBloggerForm(opts) {
    ...
    var bioHtml = '';
    if (opts.prefillBio) {
      var escapedBio = opts.prefillBio.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
      bioHtml =
        '<div style="margin-bottom:14px">' +
          '<label style="' + LABEL_CSS + '">博主简介</label>' +
          '<div style="padding:8px 10px;border-radius:8px;background:#F5F5F7;font-size:12px;color:#555;line-height:1.5;max-height:60px;overflow-y:auto;word-break:break-all">' + escapedBio + '</div>' +
        '</div>';
    }
    ...
    form.innerHTML =
      ...
      bioHtml +
      '<div style="margin-bottom:14px">' +
        '<label style="' + LABEL_CSS + '">\u8054\u7CFB\u65B9\u5F0F</label>' +
        '<input id="kol-bf-contact" type="text" placeholder="\u5FAE\u4FE1\u53F7 / \u624B\u673A\u53F7 / \u5546\u52A1\u8054\u7CFB..." style="' + INPUT_CSS + '">' +
      '</div>' +
    ...
    document.getElementById('kol-bf-save').addEventListener('click', function () {
      ...
      var contact = document.getElementById('kol-bf-contact').value.trim();
      if (!name) {
      ...
      if (opts.onSave) {
        opts.onSave(overlay, {
          ...
          name: name, followers: followers, contact: contact,
